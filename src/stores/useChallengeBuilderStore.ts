...
//       set((s) => ({ touchedSteps: { ...s.touchedSteps, [step]: true } })),
//     isStepValid: (step) => {
//       const s = get();
//       if (step === 1)
//         return Boolean(
...
//       if (!get().isGoalsEditable) return;
//       set((s) => ({ goalsAndMetrics: { ...s.goalsAndMetrics, ...data } }));
//     },
...
//     isGlobalConfiguration: () => {
//     const s = get();
//     return s.schedule.recurrenceType === "all_time" || s.rewards.participation?.whoCanParticipate === "global" || s.goalsAndMetrics.activeTab === "global";
//   },
...
export const useChallengeBuilderStore = create<ChallengeBuilderState>(
  (set, get) => ({
    ...
    isGlobalConfiguration: () => {
      const s = get();
      return (
        s.schedule.recurrenceType === "all_time" ||
        s.rewards.participation?.whoCanParticipate === "global" ||
        s.goalsAndMetrics.activeTab === "global"
      );
    },
    ...
    initializeFromApi: (apiData, challengeId) => {
      ...
      const selectedGoalTypes =
        admin.goalsAndMetrics?.selectedGoalTypes ?? ["distance"];
      ...
      const snapshot: Snapshot = {
        ...
        goalsAndMetrics: {
          selectedGoalTypes,
          activeTab: selectedGoalTypes[0] as GoalType,
          selectedGoalConfiguration: distanceGoalFromApi?.configurationType ?? "",
          selectedFrequencyConfiguration:
            frequencyGoalFromApi?.configurationType ?? "",
          selectedTimeConfiguration: timeGoalFromApi?.configurationType ?? "",
          selectedStreakConfiguration:
            streakGoalFromApi?.configurationType ?? "",
          selectedPhotoConfiguration:
            photoGoalFromApi?.configurationType ?? "",
          distanceGoal: distanceGoalFromApi
            ? {
                configurationType: distanceGoalFromApi.configurationType,
                config: distanceGoalFromApi.config,
    ...
    getUpdatePayload: () => {
      ...
      if (JSON.stringify(basicInfo) !== JSON.stringify(initialSnapshot.basicInfo)) {
        diff.basicInfo = basicInfo;
      }
      ...
      if (JSON.stringify(schedule) !== JSON.stringify(initialSnapshot.schedule)) {
        diff.schedule = schedule;
      }
