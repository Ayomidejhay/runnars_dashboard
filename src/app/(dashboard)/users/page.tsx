...
const STATUS_BADGE: Record<string, string> = {
  active:   "bg-[#ECF8F1] text-[#40B773]",
  inactive: "bg-[#FFE0B2] text-[#E65100]",
};
...
const columns: ColumnDef<User>[] = [
  {
    ...
    cell: (user) => (
      <div className="flex gap-1.5 items-center">
        ...
        />
        <div className="flex flex-col gap-1">
          <p className="font-bold text-deepblue">{user.fullName}</p>
          <p className="text-xs text-[#8E98A8] font-normal">{user.email}</p>
        </div>
      </div>
    ),
  },
  ...
  },
  {
    header: "Status",
    cell: (user) => (
      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${STATUS_BADGE[user.userStatus] ?? "bg-[#ECF8F1] text-[#40B773]"}`}>
        {user.userStatus}
      </span>
    ),
  },
...
export default function page() {
  ...
  // Cache global stats once so they don't flicker when filters change
  const [globalStats, setGlobalStats] = useState<{
    totalUsers: number;
    activeUsers: number;
    recentUsers: number;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filterValues, dateRange, customStartDate, customEndDate]);
  ...
  }, [currentPage, rowsPerPage, searchTerm, filterValues, dateRange, customStartDate, customEndDate]);

  const { data, isLoading } = useAllUsers(apiParams);
  const users: User[] = Array.isArray(data?.data?.users) ? data.data.users : [];
  const totalPages = data?.data?.pagination?.totalPages ?? 1;
  const totalUsers = data?.data?.pagination?.totalUsers ?? 0;

  // Cache global stats once on first successful load
  useEffect(() => {
    if (!globalStats && data?.data?.statistics) {
      setGlobalStats({
        totalUsers: data.data.statistics.totalUsers,
  ...
  return (
    <div className="px-10">
      <div className="flex flex-col gap-6">
        {/* Header */}
        <div className="flex justify-between items-center">
          <h1 className="capitalize text-[34px] font-bold text-deepblue">users</h1>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-3 gap-4">
          <StatCard title="Total Users" value={globalStats?.totalUsers.toLocaleString() || 0} />
          <StatCard title="Active Users" value={globalStats?.activeUsers || 0} subtitle={`${activePercentage}% of total users`} />
          <StatCard title="New Users" value={globalStats?.recentUsers || 0} subtitle="Last seven days" />
        </div>
        ...
        <DataTable
          ...
          getRowActions={(user) => [
            ...
            },
            {
              ...
              icon: "/user.svg",
              danger: true,
              onClick: () => console.log("Deactivate", user._id),
            },
          ]}
        />
      </div>
