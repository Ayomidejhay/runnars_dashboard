...
// ─────────────────────────────────────────────────────────────────────────────
// Example: Challenges page using <DataTable>
// Replace the old inline <table> block with this.
// ─────────────────────────────────────────────────────────────────────────────
...
const columns: ColumnDef<challenge>[] = [
  {
    ...
    cell: (c) => (
      <div className="flex gap-1.5 items-center">
        ...
        />
        <div className="flex flex-col gap-1">
          <p className="font-bold text-deepblue">{c.title}</p>
          <p className="text-xs text-[#8E98A8] font-normal">{c.category}</p>
        </div>
      </div>
    ),
  },
  ...
  },
  {
    ...
    align: "center",
    cell: (c) => (
      <Badge label={c.type || "N/A"} colorClass="bg-[#E8F1FD] text-[#1570EF]" />
    ),
  },
  {
    header: "Status",
    ...
    cell: (c) => (
      <Badge
        ...
        colorClass={STATUS_BADGE[c.status || ""] ?? "bg-gray-100 text-gray-800"}
      />
    ),
  },
  {
...
export default function ChallengesPage() {
  ...
  const [activeTab, setActiveTab] = useState<"all" | "featured" | "community">("all");
  const [dateRange, setDateRange] = useState("all");
  const [customStartDate, setCustomStartDate] = useState("");
  const [customEndDate, setCustomEndDate] = useState("");
  const [dateError, setDateError] = useState("");
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filterValues, activeTab, dateRange, customStartDate, customEndDate]);
  ...
  }, [currentPage, rowsPerPage, searchTerm, filterValues, activeTab, dateRange, customStartDate, customEndDate]);

  const { data, isLoading } = useAllChallenges(apiParams);
  const challenges: challenge[] = Array.isArray(data?.data?.challenges) ? data.data.challenges : [];
  const totalPages = data?.data?.pagination?.totalPages ?? 1;
  const totalCount = data?.data?.pagination?.totalCount ?? 0;
  ...
  return (
    <div className="px-10">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-[34px] font-bold">Challenges</h1>
        <div className="flex gap-4">
          <Link href="/challenges/managebadge" className="border px-4 py-2 rounded-full">
            Manage Badges
          </Link>
          <Link href="/challenges/new-challenge" className="bg-blue-600 text-white px-4 py-2 rounded-full">
            New Challenge
          </Link>
        </div>
      </div>
      ...
      <div className="border-b mb-6">
        <div className="flex gap-6">
          {(["all", "featured", "community"] as const).map((tab) => (
            <button
              key={tab}
      ...
      {/* ✅ DataTable replaces the old <table> block */}
      <DataTable
        data={challenges}
        columns={columns}
        rowKey={(c) => c._id ?? ""}
        ...
        title={`${activeTab} Challenges (${totalCount})`}
        ...
        getRowActions={(c) => [
          { label: "See Details", icon: "/eye.svg", href: `/challenges/${c._id}` },
          { label: "Edit", icon: "/edit.svg", href: `/challenges/${c._id}/edit` },
          {
            label: "Delete",
            icon: "/user.svg",
            danger: true,
