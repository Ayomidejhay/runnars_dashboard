...
const OverviewTab: React.FC<OverviewTabProps> = ({  getStatusBadge, adminChallenge }) => {
const startDateTime = combineDateAndTime(adminChallenge.scheduleAndDuration.startDate, adminChallenge.scheduleAndDuration.startTime);
  const endDateTime = combineDateAndTime(adminChallenge.scheduleAndDuration.endDate, adminChallenge.scheduleAndDuration.endTime);
...
const averageDistance =
  participants.length > 0
    ? totalDistance / participants.length
    : 0;
    ...
    <div className="flex gap-6">
      <div className="basis-[60%]">
        <div>
          ...
          <div className="mt-6 flex gap-4 w-full">
            <div className="flex-1 h-32 border border-[#E1E1E1] rounded-[16px] flex pl-6 pr-2 items-center">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-deepblue font-bold text-[24px]">{adminChallenge.publishedChallenge.participantCount}</p>
            ...
            <div className="flex-1 pl-6 pr-2 h-32 border border-[#E1E1E1] rounded-[16px] flex items-center">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-deepblue font-bold text-[24px]">{adminChallenge.publishedChallenge.progressPercentage}%</p>
                <div className="w-full bg-gray-200 h-1 rounded-full overflow-hidden mt-1">
                  <div
                    className="bg-brightblue h-full"
                    style={{ width: `${adminChallenge.publishedChallenge.progressPercentage}%` }}
                  ></div>
                </div>
              </div>
            </div>
            <div className="flex-1 pl-6 pr-2 h-32 border border-[#E1E1E1] rounded-[16px] flex items-center">
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-baseline gap-2">
                  <p className="text-deepblue font-bold text-[24px]">{totalDistance.toFixed(2)} </p>
                  <p>mi</p>
                </div>
                <p className="text-[12px] text-[#40B773]">Avg {averageDistance.toFixed(2)} mi per participant</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 p-5 bg-white rounded-2xl w-full">
            ...
            <p className="text-[12px] mt-4">{adminChallenge.basicInfo.description}</p>
          </div>
          ...
          <div className="mt-4 p-5 bg-white rounded-2xl w-full">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <div className="text-[12px] w-full flex justify-between">
                ...
                <p className="font-bold text-deepblue capitalize">{adminChallenge.publishedChallenge.challengeCategory}</p>
              </div>
              <div className="text-[12px] w-full flex justify-between">
                <p>Goal type</p>
                <p className="font-bold text-deepblue capitalize">{adminChallenge.publishedChallenge.type}</p>
              </div>
              <div className="text-[12px] w-full flex justify-between">
                <p>Tags</p>
                <div className="flex gap-1 flex-wrap">
                  {adminChallenge.basicInfo.primaryHashtags.map((tag, index) => (
                    <span
                      key={index}
      ...
      <div className="basis-[40%] flex flex-col gap-4">
        <div className="p-5 bg-white rounded-2xl w-full">
          ...
          <p className="text-[12px] mt-4 capitalize">{adminChallenge.publishedChallenge.rewards.description}</p>
        </div>
        <div className="p-5 bg-white rounded-2xl w-full">
