...
const columns: ColumnDef<EnrichedParticipant>[] = [
  {
    header: "User Name",
    cell: (p) => (
      <div className="flex items-center gap-2">
        <Image
          src={p.userDetails?.basicInfo?.profilePicture || "/profile-icon.svg"}
          alt={p.userDetails?.basicInfo?.fullName || "user"}
          width={32}
          height={32}
          className="rounded-full w-8 h-8 object-cover"
        />
        <div className="flex flex-col gap-1">
          <p className="font-bold text-deepblue">{p.userDetails?.basicInfo?.fullName}</p>
          <p className="text-xs text-[#8E98A8] font-normal">{p.userDetails?.basicInfo?.email || ""}</p>
          <p className="text-[10px] text-[#8E98A8] font-normal">{formatPets(p.userDetails?.pets)}</p>
        </div>
      </div>
    ),
  },
  {
    header: "Joined Date",
    cell: (p) => (
      <span className="font-bold text-deepblue">{formatJoinedDate(p.joinedAt)}</span>
    ),
  },
  {
    header: "Status",
    cell: (p) => (
      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${STATUS_BADGE[p.status] ?? "bg-gray-100 text-gray-500"}`}>
        {p.status}
      </span>
    ),
  },
    ...
    cell: () => (
      <div className="flex items-center gap-2">
        <div className="w-[120px] h-[6px] bg-gray-200 rounded-full overflow-hidden">
          <div className="bg-brightblue h-full" style={{ width: "0%" }} />
        </div>
      </div>
    ),
  },
  {
    ...
    cell: (p) => (
      <Link
        href={`/users/${p.userId}`}
        className="w-[85px] h-[36px] rounded-full border bg-transparent border-[#E1E1E1] flex items-center justify-center text-xs cursor-pointer"
      >
        View details
      </Link>
    ),
...
export default function ParticipantsTab({ adminChallenge }: ParticipantsTabProps) {
  const { id } = useParams();

  const { data } = useChallengeOverview(id as string);

  const participants: Participant[] = data?.data?.participantStatistics ?? [];
  ...
  const enrichedParticipants: EnrichedParticipant[] = useMemo(
    () => participants.map((p) => ({ ...p, userDetails: userMap[p.userId] ?? null })),
    [participants, userMap]
  );
  ...
  const filteredParticipants = useMemo(() => {
    return enrichedParticipants.filter((p) => {
      const fullName = p.userDetails?.basicInfo?.fullName?.toLowerCase() ?? "";
      const matchesSearch = fullName.includes(searchTerm.toLowerCase());
      const matchesStatus = filterValues.status === "all" || p.status === filterValues.status;
      return matchesSearch && matchesStatus;
    });
  }, [enrichedParticipants, searchTerm, filterValues]);

  // Client-side pagination — pass only the current page slice to DataTable
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { title: "Participants",          value: data?.data?.participantMetrics?.totalParticipants || 0,   subtitle: "" },
          { title: "Active Participants",   value: data?.data?.participantMetrics?.activeParticipants || 0,  subtitle: "Currently active" },
          { title: "Completed Challenge",   value: data?.data?.participantMetrics?.completedParticipants || 0, subtitle: "Marked complete" },
        ].map((item, idx) => (
          <div key={idx} className="border border-[#E1E1E1] p-6 rounded-[16px] bg-transparent flex flex-col gap-1 mt-6">
            <span className="text-sm text-gray">{item.title}</span>
            <div className="text-2xl font-bold text-deepblue">{item.value}</div>
            <p className="text-xs text-[#40B773]">{item.subtitle}</p>
          </div>
        ))}
      </div>

      {/* ✅ DataTable with client-side paginated slice */}
      <DataTable
