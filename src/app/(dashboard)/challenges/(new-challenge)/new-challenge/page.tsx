...
//           {/* {step === 3 && <Schedule />} */}
//           {step === 3 &&
//   !useChallengeBuilderStore.getState().isGlobalConfiguration() && (
//     <Schedule />
//   )}
...
export default function Page() {
  ...
  // Determine if Schedule should be skipped
  const isGlobal = goalsAndMetrics.selectedGoalTypes.includes("global");

  // Define effective steps dynamically
  const effectiveSteps = ["Basic Info", "Goal & Metric", ...(isGlobal ? [] : ["Schedule"]), "Rewards"];
  ...
  const nextStep = () => {
    ...
    setStep(Math.min(step + 1, effectiveSteps.length));
  };
  const prevStep = () => setStep(Math.max(step - 1, 1));
  ...
  // Map real step to effectiveSteps index
  const currentStepIndex = isGlobal && step === 3 ? 2 : step - 1;
    ...
    <div>
      ...
      <div className="flex border-b border-[#E1E1E1]">
        <div className="basis-[60%] pt-6 px-10 flex flex-col gap-6">
          {step === 1 && <BasicInfo />}
          {step === 2 && <GoalMetric />}
          {step === 3 && !isGlobal && <Schedule />}
          {step === (isGlobal ? 3 : 4) && <Reward />}
        </div>
      ...
      {/* NAVIGATION */}
      <div className={`flex mt-6 px-10 ${currentStepIndex === 0 ? "justify-end" : "justify-between"}`}>
        {step > 1 && (
          <button
            onClick={prevStep}
            className="bg-brightblue rounded-[32px] text-white w-[90px] h-[48px]"
        ...
        {currentStepIndex < effectiveSteps.length - 1 && (
          <button
            ...
            disabled={!isStepValid(step)}
            className={`rounded-[32px] w-[190px] h-[48px] ${
              isStepValid(step) ? "bg-brightblue text-white" : "bg-gray-300 text-gray-500 cursor-not-allowed"
            }`}
          >
            Next: {effectiveSteps[currentStepIndex + 1]} →
          </button>
        )}
